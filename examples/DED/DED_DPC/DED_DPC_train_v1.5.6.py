# -*- coding: utf-8 -*-
"""
Created on Thu Oct 16 13:47:10 2025
...
# =========================
# Construct the Objective and Constraints  (Neuromancer >= 1.5.x)
# =========================

# Symbolic variables that refer to node outputs/inputs by name
X_pred = nm.constraint.variable(f"X_pred_{dynamics_model.name}")  # shape: [T, B, nx]
U_pred = nm.constraint.variable(f"U_pred_{policy.name}")          # shape: [T, B, nu]
coefsf = nm.constraint.variable("coefsf")                          # metadata coefs (from dataset)
Up_sym = nm.constraint.variable("Up")                              # previous control sequence

# ---- Objective (reference_loss) ----
# Original Objective_fn(x,u,c):
#   mean( || c[:,:,0]*x[:,:,2] ||_1  +  sum over t of || c[:,:,[1,2]] * u ||_1 )
ref_expr = (
    torch.linalg.norm(coefsf[:, :, 0] * X_pred[:, :, 2], ord=1, dim=0)
    + torch.linalg.norm(coefsf[:, :, [1, 2]] * U_pred, ord=1, dim=0).sum(1)
).mean()
reference_loss = ref_expr.minimize(weight=1.0, name="ref_loss")

# ---- Ramping constraint penalty ----
c_rmp_tol = torch.tensor([rmp_tol], requires_grad=False, device=device)

def _shift_first_dim(u, up):
    # up: [1, B, nu] to match the concat at the front
    spp = list(u.shape); spp[0] = 1
    up_reshaped = torch.reshape(up[0, :, :], spp)
    return torch.cat((u, up_reshaped), dim=0)

u_with_prev = _shift_first_dim(U_pred, Up_sym)
rmp_pos = torch.linalg.norm(
    F.relu(u_with_prev[1:, :, :] - u_with_prev[:-1, :, :] - c_rmp_tol),
    ord=1, dim=0
).sum(1)
rmp_neg = torch.linalg.norm(
    F.relu(-u_with_prev[1:, :, :] + u_with_prev[:-1, :, :] - rmp_tol),
    ord=1, dim=0
).sum(1)
rmp_expr = (rmp_pos + rmp_neg).mean()
Control_ramping_pen = rmp_expr.minimize(weight=rmp_cns_weight, name="ramping_cnstrnt")

# ---- Initial control consistency ----
init_expr = torch.linalg.norm(Up_sym[0, :, :] - U_pred[0, :, :], ord=2, dim=1).mean()
Control_initial_pen = init_expr.minimize(weight=init_cntrl_weight, name="cntrl_init_cnstrnt")

# ---- Bounds on generator inputs u ----
u_min_t = torch.tensor(u_min, requires_grad=False, device=device)
u_max_t = torch.tensor(u_max, requires_grad=False, device=device)

lb_expr = torch.linalg.norm(F.relu(-U_pred + u_min_t), ord=1, dim=0).sum(1).mean()
ub_expr = torch.linalg.norm(F.relu(U_pred - u_max_t),  ord=1, dim=0).mean()

Control_lower_bound_penalty = lb_expr.minimize(weight=u_min_weight, name="control_lower_bound")
Control_upper_bound_penalty = ub_expr.minimize(weight=u_max_weight, name="control_upper_bound")

# ---- Bounds on generator frequencies (states) ----
omega_max_t = torch.tensor(omega_max, requires_grad=False, device=device)
omega_min_t = torch.tensor(omega_min, requires_grad=False, device=device)

omega_ub_expr = torch.linalg.norm(F.relu(X_pred[:, :, [0, 1]] - omega_max_t), ord=1, dim=0).sum(1).mean()
omega_lb_expr = torch.linalg.norm(F.relu(-X_pred[:, :, [0, 1]] + omega_min_t), ord=1, dim=0).sum(1).mean()

omega_upper_bound_penalty = omega_ub_expr.minimize(weight=omega_max_weight, name="omega_upper_bound")
omega_lower_bound_penalty = omega_lb_expr.minimize(weight=omega_min_weight, name="omega_lower_bound")

# =========================
# Build The Neuromancer Problem  (new signature)
# =========================
...
components = [estimator, StO_map, policy, dynamics_model]
model = Problem(nodes=components, loss=loss).to(device)
